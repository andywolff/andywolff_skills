...
Records ONLY broadly applicable, workspace-agnostic guidelines, design patterns,
and compiler/tooling constraints. Document project-specific context directly in
PR descriptions or code comments instead.

* **Organization**: Group items under their domain headings (`###`) and separate
  with `---`. Update the **Quick Reference Checklist** to match any changes.
* **Skills vs. Worklog**: Use this worklog for design rules, compiler pitfalls,
  and environment setup constraints. Use **Personal Skills** for step-by-step
  procedures, CLI wrappers, or automated scripts.

### Strict Exclusions Checklist (DO NOT ADD):
* **No Project-Specific Details**: Never reference specific test suites, local
  file paths, PR links, bug IDs, or individual build configurations.
* **No Hard Data or Metrics**: Never log baseline stats, run success rates,
  dates/times, or transient builder execution times.
* **No Redundant Skill Tracking**: If a script, CLI shortcut, or procedure was
  added or modified inside a skill, do not document or link it here. The skill
...
- [ ] **Local Linter Masking**: Delete untracked local
  `analysis_options.yaml` files generated by `flutter create` to prevent
  overriding repo-wide rules during analysis runs.
